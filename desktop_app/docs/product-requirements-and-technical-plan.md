...
实际格子数组不直接展开成巨大 JSON. 当前项目格式按同目录同名文件对保存: `*.perler.json` 保存上述元数据, 编辑器颜色栏, 紧凑图纸预览, CSV 文件名, 字节数和内容校验值; `.csv` 使用带坐标的色号矩阵保存实际格子. JSON 是项目入口, 打开时必须同时校验同名 CSV 的内容校验值, 尺寸, 色卡和逐格数据. 最近项目, 上次项目目录和上次导出目录保存在系统应用配置目录内带 schema 版本的 `user-state.json`; 扩展字段集中放入 `extensions` 对象. 未来需要保存母图或恢复点时, 通过项目 schema 升级或扩展目录增加, 不改变既有文件的解释.
...
- 开关旁显示检测结果, CLI 版本和隔离权限说明. 用户可以为当前 Codex 子进程填写 HTTP, HTTPS 或 SOCKS5 网络代理.
- 第一次执行前要求用户确认输入图片, 输出目录和允许写入范围.
- 任务面板显示阶段, JSONL 进度, 运行时间和取消按钮. 运行输出只提取 JSONL 中允许展示的文字字段, 在固定高度的可滚动面板中持续更新, 不展示原始 JSON 结构.
